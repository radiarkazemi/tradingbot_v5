...
class GridEngine(threading.Thread):
    ...
    def __init__(
        ...
        self._wide_spread_since = None  # tracks when wide spread started
    ...
    def run(self):
        ...
        self._log(
            f"📐 Symbol: {self.symbol}  Pip: {self._pip}  Digits: {self._digits}", "INFO")
    ...
    def _tick(self):
        ...
        if tick_age > 120:
            self._status(
                f"Market closed (last tick {int(tick_age/60)}min ago)")
            self._emit_stats()
            return
        ...
        if day != self._last_day:
            if self._active:
                self._close_all("day end")
            self._asian_h = 0.0
            self._asian_l = float('inf')
            self._asian_ready = False
            self._session_fired_today = False
            self._daily_sl_hit = False
            self._wide_spread_since = None
            acct = mt5.account_info()
            self._bal_day = acct.balance if acct else self._bal_day
            self._last_day = day
            self._log(
                f"📅 New day {day} | Balance: ${self._bal_day:.2f}", "INFO")
        ...
        if self._bal_day > 0:
            ...
            if daily_loss_pct <= -self.daily_sl_pct:
                ...
                self._log(
                    f"⛔ Daily SL hit ({daily_loss_pct:.1f}%) — no more sessions today", "WARN")
                if self._active:
                    self._close_all("daily SL")
                self._status("⛔ Daily SL reached — waiting for tomorrow")
                self._emit_stats()
                return

        # ── Build Asian range (skipped if asian_end_hour=0) ──────────
        if self.asian_end_hour > 0 and h < self.asian_end_hour:
            if bid > self._asian_h:
                self._asian_h = bid
            if bid < self._asian_l:
                self._asian_l = bid
            self._asian_ready = self._asian_h > 0 and self._asian_l < float(
                'inf')
        ...
        if not self._active and not self._session_fired_today:
            ...
            if spread_pips <= self.max_spread_pips:
                ...
                if not use_asian:
                    self._log(
                        "⚠ No Asian range — using current price as anchor", "WARN")
                self._start_session(bid, ask, use_asian)
                self._session_fired_today = True
            else:
                # Wide spread — wait up to 5 minutes then start anyway
                if not hasattr(self, '_wide_spread_since') or self._wide_spread_since is None:
                    self._wide_spread_since = datetime.now()
                    self._log(
                        f"⚠ Spread {spread_pips:.1f}pip — waiting for it to narrow (max 5 min)...", "WARN")
                elapsed = (datetime.now() -
                           self._wide_spread_since).total_seconds()
                if elapsed > 300:  # 5 minutes passed — start anyway
                    self._log(
                        f"⚠ Spread still {spread_pips:.1f}pip after 5min — starting anyway", "WARN")
                    use_asian = self._asian_ready
                    self._start_session(bid, ask, use_asian)
                    self._session_fired_today = True
                    self._wide_spread_since = None
                    ...
                    self._status(
                        f"Spread {spread_pips:.1f}pip — waiting ({int(elapsed)}s / 300s)...")
            self._emit_stats()
            return
    ...
    def _start_session(self, bid: float, ask: float, use_asian: bool):
        ...
        self._atr_val = atr
        self._step = max(atr * self.step_atr_mult,
                         self.min_step_pips * self._pip)
        self._sl_dist = max(atr * self.sl_atr_mult,
                            self.min_sl_pips * self._pip)
        ...
        if use_asian and self._asian_h > 0 and self._asian_l < float('inf'):
            ...
            self._anchor = _snap(
                (bid + ask) / 2.0, self._tick_size, self._digits)
        ...
        buy_sl = _snap(self._anchor - self._sl_dist,
                       self._tick_size, self._digits)
        sell_sl = _snap(self._anchor + self._sl_dist,
                        self._tick_size, self._digits)
        target = self._bal_start * self.target_pct / 100.0

        self._log("=" * 54, "INFO")
        self._log(
            f"▶ SESSION #{self._session_n} | Balance: ${self._bal_start:.2f} | Target: ${target:.2f}", "NEW")
        if use_asian:
            ...
            self._log(
                f"📌 Anchor={self._anchor:.{self._digits}f} (current mid)", "INFO")
        self._log(
        ...
        for i in range(self.grid_count):
            price = _snap(self._anchor + (i+1)*self._step,
                          self._tick_size, self._digits)
            ...
            if res and res.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                self._log(
                    f"   🟢 {tag} #{i+1} @ {price:.{self._digits}f}  SL={buy_sl:.{self._digits}f}", "NEW")
            else:
                rc = res.retcode if res else "N/A"
                self._log(
                    f"   ❌ BUY #{i+1} FAILED @ {price:.{self._digits}f}  retcode={rc}", "ERROR")
        ...
        for i in range(self.grid_count):
            ...
            if res and res.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                self._log(
                    f"   🔴 {tag} #{i+1} @ {price:.{self._digits}f}  SL={sell_sl:.{self._digits}f}", "NEW")
            else:
                rc = res.retcode if res else "N/A"
                self._log(
                    f"   ❌ SELL #{i+1} FAILED @ {price:.{self._digits}f}  retcode={rc}", "ERROR")
        ...
        if total == 0:
            ...
            self._status(
                f"Grid active — {pb}B + {ps}S pending | Target ${target:.2f}")
    ...
    def _check_side_lock(self, bid: float, ask: float):
        ...
        if buys and not sells:
            self._locked_side = "BUY"
            cancel = "SELL"
        elif sells and not buys:
            self._locked_side = "SELL"
            cancel = "BUY"
        elif buys and sells:
            ...
            self._log(
                f"⚡ Both sides ({len(buys)}B/{len(sells)}S) — locking {self._locked_side}", "WARN")

        self._log(
            f"🔒 Locked: {self._locked_side} | Cancelling {cancel} pending", "NEW")
    ...
    def _update_stops(self, bid: float, ask: float):
        ...
        be_pips = self.be_atr_mult * \
            (self._atr_val / self._pip) if self._pip > 0 else 5
        any_be = any(
        ...
        for p in our:
            ...
            if is_buy:
                ...
                if (pp >= be_pips or (any_be and pp >= 0)) and p.ticket not in self._be_done:
                    be_sl = _snap(p.price_open + self._tick_size,
                                  self._tick_size, self._digits)
                    if be_sl > new_sl:
                        new_sl = be_sl
            else:
                tsl = _snap(cur_p + self._trail, self._tick_size, self._digits)
                if cur_sl == 0 or tsl < new_sl:
                    new_sl = tsl
                pp = (p.price_open - ask) / self._pip
                if (pp >= be_pips or (any_be and pp >= 0)) and p.ticket not in self._be_done:
                    be_sl = _snap(p.price_open - self._tick_size,
                                  self._tick_size, self._digits)
                    if cur_sl == 0 or be_sl < new_sl:
                        new_sl = be_sl
            ...
            if abs(new_sl - cur_sl) > self._tick_size * 2:
                if _modify_sl(p.ticket, new_sl, self.symbol):
                    ...
                    if be_triggered and p.ticket not in self._be_done:
                        ...
                        self._log(
                            f"   ✅ BE: #{p.ticket} {'BUY' if is_buy else 'SELL'} → {new_sl:.{self._digits}f}", "NEW")
    ...
    def _check_exits(self, bid: float, ask: float):
        ...
        if not our and not our_ord:
            ...
            now_utc = datetime.now(tz=timezone.utc)
            h = now_utc.hour
            # asian_end_hour=0 means trade anytime
            in_window = (self.asian_end_hour == 0 or h >= self.asian_end_hour) \
                and h < self.trade_end_hour
            if in_window:
                self._log("ℹ All SL/BE closed — starting new session", "INFO")
                self._reset_session()
        ...
        # ── TRUE net P&L = realized (balance moved) + floating (open positions) ──
        # This accounts for: positions closed at SL, lock-closes, commissions paid
        acct = mt5.account_info()
        cur_bal = acct.balance if acct else self._bal_start
        realized = cur_bal - self._bal_start           # locked-in P&L this session
        # open positions floating
        floating = sum(p.profit + p.swap for p in our)
        net_pnl = realized + floating                  # TRUE session P&L

        target = self._bal_start * self.target_pct / 100.0
        if net_pnl > self._peak:
            self._peak = net_pnl

        # ── Stuck position check ──────────────────────────────────
        if not our_ord and our:
            all_be = all(p.ticket in self._be_done for p in our)
            if all_be and len(our) <= 2:
                self._log(
                    "♻ Positions stuck at BE, no pending — closing and restarting", "INFO")
                self._close_all("stuck at BE")
                return

        # Pullback guard — uses net_pnl (true picture)
        if self._peak >= target * 0.4 and self._peak > 0:
            drop = (self._peak - net_pnl) / self._peak * 100
            if drop >= self.pullback_pct:
                self._log(
                    f"📉 Pullback guard: {drop:.1f}% drop — closing", "WARN")
                self._close_all(f"pullback {drop:.0f}%")
                return

        # Target — uses net_pnl so it's pure profit after all costs
        if net_pnl >= target:
            self._log(
                f"🎯 Target hit! Net P&L ${net_pnl:.2f} ≥ ${target:.2f}  "
                f"(realized ${realized:+.2f} + floating ${floating:+.2f})", "NEW"
            )
            self._close_all(f"target ${net_pnl:.2f}")
        ...
        for attempt in range(5):
            ...
            for p in our:
                ...
                if res and res.retcode == mt5.TRADE_RETCODE_DONE:
                    closed += 1
                    self._log(
                        f"   {'🟢' if pnl>=0 else '🔴'} Closed #{p.ticket}  P&L: ${pnl:+.2f}", "INFO")
                else:
                    failed += 1
                    self._log(
                        f"   ⚠ Close #{p.ticket} failed: {res.retcode if res else 'N/A'}", "WARN")
            if failed == 0:
                break
            time.sleep(0.3)
        ...
        for o in (mt5.orders_get(symbol=self.symbol) or []):
            if o.magic == self.magic and o.type in target:
                if mt5.order_send({"action": mt5.TRADE_ACTION_REMOVE, "order": o.ticket}):
                    n += 1
        if n:
            self._log(f"   🗑 Cancelled {n} {side} pending orders", "INFO")
    ...
    def _emit_stats(self):
        ...
        # TRUE net P&L = realized (balance moved since session start) + floating
        realized = (acct.balance - self._bal_start) if acct else 0.0
        net_pnl = realized + floating
        target = self._bal_start * self.target_pct / 100.0
        elapsed = (
            datetime.now() - self._sess_start).total_seconds() if self._sess_start else 0.0
        pct = (net_pnl / target * 100) if target > 0 else 0.0
        ...
        self._stats({
            ...
            "pending":      len(our_ord),
            "floating":     net_pnl,    # true net P&L (realized + floating)
            "peak":         self._peak,
            "target":       target,
            "progress_pct": max(0.0, min(100.0, pct)),
