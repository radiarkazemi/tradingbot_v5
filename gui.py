...
def _card(title: str, accent: str, big=False):
    ...
    lay = QVBoxLayout(card); lay.setContentsMargins(10,6,10,8); lay.setSpacing(2)
    tl = QLabel(title.upper())
    tl.setStyleSheet(f"color:{C['txt3']};font-size:9px;font-weight:bold;letter-spacing:1px;border:none;")
    lay.addWidget(tl)
    vl = QLabel("—")
    vl.setStyleSheet(f"color:{C['txt']};font-family:Consolas;font-size:{'16' if big else '13'}px;font-weight:bold;border:none;")
    lay.addWidget(vl)
    return card, vl
...
class Sparkline(QWidget):
    ...
    def push(self, v):
        self._data.append(v)
        if len(self._data) > 200: self._data = self._data[-200:]
        self.update()
    ...
    def paintEvent(self, _):
        ...
        fc  = QColor(col); fc.setAlpha(28)
        path = QPainterPath()
        path.moveTo(*pt(0, self._data[0]))
        for i, v in enumerate(self._data[1:], 1): path.lineTo(*pt(i, v))
...
class GUI(QMainWindow):
    ...
    def __init__(self):
        ...
        t = QTimer(self); t.timeout.connect(self._refresh_price); t.start(1000)
        t2 = QTimer(self); t2.timeout.connect(self._refresh_tables); t2.start(500)
    ...
    def _build_ui(self):
        ...
        sp.setSizes([320, 740]); sp.setChildrenCollapsible(False)
        root.addWidget(sp)
        sb = self.statusBar()
        sb.setStyleSheet(f"background:{C['panel']};color:{C['txt2']};border-top:1px solid {C['border']};font-size:11px;")
        self.lbl_sb = QLabel("Ready"); self.lbl_sb.setStyleSheet(f"color:{C['txt2']};")
    ...
    def _left(self):
        ...
        lay.setContentsMargins(4,6,8,6); lay.setSpacing(8)

        t = QLabel("TraderBot v5")
        t.setStyleSheet(f"color:{C['gold']};font-size:18px;font-weight:bold;padding:2px 0;")
        s = QLabel("Smart ATR Breakout Grid")
        s.setStyleSheet(f"color:{C['txt2']};font-size:10px;padding-bottom:4px;")
        lay.addWidget(t); lay.addWidget(s); lay.addWidget(_hline())

        # Connection
        conn = QGroupBox("MT5 Connection")
        cg = QVBoxLayout(conn); cg.setSpacing(5)
        def _field(lbl, w):
            r = QHBoxLayout()
            lb = QLabel(lbl); lb.setFixedWidth(68)
            lb.setStyleSheet(f"color:{C['txt2']};font-size:11px;")
            r.addWidget(lb); r.addWidget(w); cg.addLayout(r)
        self.inp_login = QLineEdit(str(cfg.MT5_LOGIN))
        self.inp_pass  = QLineEdit(cfg.MT5_PASSWORD); self.inp_pass.setEchoMode(QLineEdit.Password)
        self.inp_srv   = QLineEdit(cfg.MT5_SERVER)
        _field("Login", self.inp_login); _field("Password", self.inp_pass)
        ...
        sl = QVBoxLayout(sg); sl.setSpacing(5)
        cr = QHBoxLayout()
        ...
        self.lbl_price.setStyleSheet(f"color:{C['cyan']};font-family:Consolas;font-size:15px;font-weight:bold;")
        self.lbl_price.setAlignment(Qt.AlignRight|Qt.AlignVCenter)
        cr.addWidget(self.sym_combo, 1); cr.addWidget(self.lbl_price)
        sl.addLayout(cr)
        cust = QHBoxLayout()
        lc = QLabel("Custom:"); lc.setStyleSheet(f"color:{C['txt2']};font-size:11px;"); lc.setFixedWidth(52)
        self.inp_custom = QLineEdit(); self.inp_custom.setPlaceholderText("e.g. XAUUSD_o…")
        self.inp_custom.returnPressed.connect(self._on_custom_sym)
        bu = QPushButton("Use"); bu.setFixedWidth(44); bu.clicked.connect(self._on_custom_sym)
        cust.addWidget(lc); cust.addWidget(self.inp_custom, 1); cust.addWidget(bu)
        sl.addLayout(cust)
        lay.addWidget(sg)

        # Grid settings
        gg = QGroupBox("Grid Settings  (Optimized)")
        gl = QGridLayout(gg); gl.setSpacing(6); gl.setColumnStretch(1,1)
        def lbl(t):
            l = QLabel(t); l.setStyleSheet(f"color:{C['txt2']};font-size:11px;"); return l

        self.sp_count = QSpinBox()
        self.sp_count.setRange(1,50); self.sp_count.setValue(cfg.GRID_COUNT)
        self.sp_count.setSuffix(" each side")

        self.sp_step  = QDoubleSpinBox()
        self.sp_step.setRange(0.05,2.0); self.sp_step.setSingleStep(0.05)
        self.sp_step.setDecimals(2); self.sp_step.setValue(cfg.GRID_STEP_ATR)
        self.sp_step.setPrefix("ATR × ")

        self.sp_lot   = QDoubleSpinBox()
        self.sp_lot.setRange(0.01,100.0); self.sp_lot.setSingleStep(0.01)
        self.sp_lot.setDecimals(2); self.sp_lot.setValue(cfg.LOT_SIZE)
        self.sp_lot.setPrefix("lot  ")

        gl.addWidget(lbl("Count"),   0,0); gl.addWidget(self.sp_count, 0,1)
        gl.addWidget(lbl("Step"),    1,0); gl.addWidget(self.sp_step,  1,1)
        gl.addWidget(lbl("Lot"),     2,0); gl.addWidget(self.sp_lot,   2,1)
        lay.addWidget(gg)

        # Stop management
        stg = QGroupBox("Stop Management  (Optimized)")
        stl = QGridLayout(stg); stl.setSpacing(6); stl.setColumnStretch(1,1)

        self.sp_sl    = QDoubleSpinBox()
        self.sp_sl.setRange(0.5,5.0); self.sp_sl.setSingleStep(0.25)
        self.sp_sl.setDecimals(2); self.sp_sl.setValue(cfg.SL_ATR_MULT)
        self.sp_sl.setPrefix("ATR × ")

        self.sp_trail = QDoubleSpinBox()
        self.sp_trail.setRange(0.1,2.0); self.sp_trail.setSingleStep(0.1)
        self.sp_trail.setDecimals(2); self.sp_trail.setValue(cfg.TRAIL_ATR_MULT)
        self.sp_trail.setPrefix("ATR × ")

        self.sp_be    = QDoubleSpinBox()
        self.sp_be.setRange(0.05,1.0); self.sp_be.setSingleStep(0.05)
        self.sp_be.setDecimals(2); self.sp_be.setValue(cfg.BE_ATR_MULT)
        self.sp_be.setPrefix("ATR × ")

        stl.addWidget(lbl("Hard SL"),    0,0); stl.addWidget(self.sp_sl,    0,1)
        stl.addWidget(lbl("Trail Stop"), 1,0); stl.addWidget(self.sp_trail, 1,1)
        stl.addWidget(lbl("Breakeven"),  2,0); stl.addWidget(self.sp_be,    2,1)
        lay.addWidget(stg)

        # Session settings
        sess = QGroupBox("Session Settings")
        sel  = QGridLayout(sess); sel.setSpacing(6); sel.setColumnStretch(1,1)

        self.sp_target = QDoubleSpinBox()
        self.sp_target.setRange(0.1,20.0); self.sp_target.setSingleStep(0.5)
        self.sp_target.setDecimals(1); self.sp_target.setValue(cfg.TARGET_PCT)
        self.sp_target.setSuffix(" % profit")

        self.sp_dsl    = QDoubleSpinBox()
        self.sp_dsl.setRange(1.0,20.0); self.sp_dsl.setSingleStep(1.0)
        self.sp_dsl.setDecimals(1); self.sp_dsl.setValue(cfg.DAILY_SL_PCT)
        self.sp_dsl.setSuffix(" % daily SL")

        self.sp_pb     = QDoubleSpinBox()
        self.sp_pb.setRange(10.0,80.0); self.sp_pb.setSingleStep(5.0)
        self.sp_pb.setDecimals(1); self.sp_pb.setValue(cfg.PULLBACK_PCT)
        self.sp_pb.setSuffix(" % pullback")

        sel.addWidget(lbl("Target"),   0,0); sel.addWidget(self.sp_target, 0,1)
        sel.addWidget(lbl("Daily SL"), 1,0); sel.addWidget(self.sp_dsl,    1,1)
        sel.addWidget(lbl("Pullback"), 2,0); sel.addWidget(self.sp_pb,     2,1)
        lay.addWidget(sess)

        # Buttons
        self.btn_start = QPushButton("▶  Start Bot")
        self.btn_start.setObjectName("btn_start"); self.btn_start.setMinimumHeight(42)
        self.btn_start.clicked.connect(self._on_start)
        self.btn_stop = QPushButton("⏹  Stop && Close All")
        self.btn_stop.setObjectName("btn_stop"); self.btn_stop.setMinimumHeight(42)
        self.btn_stop.setEnabled(False); self.btn_stop.clicked.connect(self._on_stop)
    ...
    def _tab_dashboard(self):
        ...
        for c in (c1,c2,c3,c4):
            c.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed); r1.addWidget(c)
        lay.addLayout(r1)
        ...
        for c in (c5,c6,c7,c8):
            c.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed); r2.addWidget(c)
        lay.addLayout(r2)

        # Progress bar
        pf = QFrame(); pf.setStyleSheet(f"QFrame{{background:{C['card']};border:1px solid {C['border']};border-radius:5px;}}")
        pfl = QVBoxLayout(pf); pfl.setContentsMargins(10,8,10,10); pfl.setSpacing(4)
        ph = QHBoxLayout()
        pl = QLabel("Profit Progress"); pl.setStyleSheet(f"color:{C['txt2']};font-size:10px;font-weight:bold;")
        self.lbl_pct = QLabel("0.0%"); self.lbl_pct.setStyleSheet(f"color:{C['green']};font-family:Consolas;font-size:11px;font-weight:bold;")
        ph.addWidget(pl); ph.addStretch(); ph.addWidget(self.lbl_pct)
        pfl.addLayout(ph)
        ...
        # Info bar
        ib = QFrame(); ib.setStyleSheet(f"QFrame{{background:{C['card']};border:1px solid {C['border']};border-radius:5px;}}")
        ibl = QHBoxLayout(ib); ibl.setContentsMargins(12,8,12,8); ibl.setSpacing(16)
        ...
        for lb in (self.lbl_anchor, self.lbl_locked, self.lbl_asian, self.lbl_daily):
            lb.setStyleSheet(f"color:{C['txt2']};font-family:Consolas;font-size:11px;")
            ibl.addWidget(lb)
        ibl.addStretch()
        lay.addWidget(ib)

        # ATR info bar
        ab = QFrame(); ab.setStyleSheet(f"QFrame{{background:{C['card']};border:1px solid {C['border']};border-radius:5px;}}")
        abl = QHBoxLayout(ab); abl.setContentsMargins(12,8,12,8); abl.setSpacing(16)
        ...
        # Sparkline
        sf = QFrame(); sf.setStyleSheet(f"QFrame{{background:{C['card']};border:1px solid {C['border']};border-radius:5px;}}")
        sfl = QVBoxLayout(sf); sfl.setContentsMargins(10,8,10,8); sfl.setSpacing(4)
        sl = QLabel("Equity Trend"); sl.setStyleSheet(f"color:{C['txt2']};font-size:10px;font-weight:bold;")
        sfl.addWidget(sl)
        self.sparkline = Sparkline(); sfl.addWidget(self.sparkline)
        lay.addWidget(sf)
        lay.addStretch()
        return w
    ...
    def _tab_positions(self):
        ...
        lp.setStyleSheet(f"color:{C['txt3']};font-size:9px;font-weight:bold;letter-spacing:2px;")
        lay.addWidget(lp)
        self.pos_table = QTableWidget(); self.pos_table.setColumnCount(7)
        self.pos_table.setHorizontalHeaderLabels(["Ticket","Type","Lot","Open","Current","P&L","Comment"])
        hh = self.pos_table.horizontalHeader(); hh.setSectionResizeMode(QHeaderView.Stretch)
        ...
        lo.setStyleSheet(f"color:{C['txt3']};font-size:9px;font-weight:bold;letter-spacing:2px;")
        lay.addWidget(lo)
        self.ord_table = QTableWidget(); self.ord_table.setColumnCount(5)
        self.ord_table.setHorizontalHeaderLabels(["Ticket","Type","Lot","Price","Comment"])
        hh2 = self.ord_table.horizontalHeader(); hh2.setSectionResizeMode(QHeaderView.Stretch)
        ...
        # Footer
        foot = QFrame(); foot.setStyleSheet(f"QFrame{{background:{C['card']};border:1px solid {C['border']};border-radius:5px;}}")
        fl = QHBoxLayout(foot); fl.setContentsMargins(12,8,12,8); fl.setSpacing(20)
        ...
        for lb in (self.lbl_sbuy, self.lbl_ssell, self.lbl_spnl):
            lb.setStyleSheet(f"color:{C['txt2']};font-family:Consolas;font-size:11px;")
            fl.addWidget(lb)
        fl.addStretch(); lay.addWidget(foot)
        return w
    ...
    def _tab_log(self):
        ...
        tb = QHBoxLayout()
        hdr = QLabel("SESSION LOG"); hdr.setStyleSheet(f"color:{C['txt3']};font-size:9px;font-weight:bold;letter-spacing:2px;")
        bc = QPushButton("Clear"); bc.setFixedWidth(70); bc.clicked.connect(lambda: self.log_view.clear())
        tb.addWidget(hdr); tb.addStretch(); tb.addWidget(bc)
        lay.addLayout(tb); lay.addWidget(_hline())
    ...
    def _on_custom_sym(self):
        ...
        if sym: self.inp_custom.setText(sym); self.lbl_price.setText("—")
    ...
    def _set_inputs(self, en):
        for w in (self.sym_combo, self.inp_login, self.inp_pass, self.inp_srv,
                  ...
                  self.sp_trail, self.sp_be, self.sp_target, self.sp_dsl, self.sp_pb):
            w.setEnabled(en)

    # ── Start / Stop ──────────────────────────────────────────────
    def _on_start(self):
        if self._engine and self._engine.is_alive(): return
        sym = self._sym()
        if not sym: return
    ...
    def _on_stats(self, d):
        ...
        self.lbl_locked.setStyleSheet(f"color:{lk_col};font-family:Consolas;font-size:11px;")
        self.lbl_locked.setText(f"Locked: {locked if locked else '—'}")
        if asian_h > 0:
            self.lbl_asian.setText(f"Asian: {asian_l:.2f}–{asian_h:.2f}  ({asian_r:.0f}pip)")
        self.lbl_daily.setStyleSheet(f"color:{dp_col};font-family:Consolas;font-size:11px;")
        ...
        if eq > 0: self.sparkline.push(eq)
    ...
    def _on_done(self, reason):
        ...
        for lb in (self.c_float, self.c_buy, self.c_sell, self.c_peak):
            lb.setText("—")
        self.progress.setValue(0); self.lbl_pct.setText("0.0%")

    # ── Table refresh ─────────────────────────────────────────────
    def _refresh_tables(self):
        if not self._engine or not self._engine.is_alive(): return
        try:
            import MetaTrader5 as mt5
            if mt5.account_info() is None: return  # MT5 not connected
            sym = self._sym()
            ...
            for r, p in enumerate(our):
                ...
                if p.type==mt5.ORDER_TYPE_BUY: bp+=pnl; bc+=1
                else: sp+=pnl; sc+=1
            ...
            net = bp+sp; nc = C['green'] if net>=0 else C['red']
            self.lbl_sbuy.setText(f"Buys: {bc}  (${bp:+.2f})")
            self.lbl_ssell.setText(f"Sells: {sc}  (${sp:+.2f})")
            self.lbl_spnl.setStyleSheet(f"color:{nc};font-family:Consolas;font-size:11px;")
    ...
    def _refresh_price(self):
        try:
            ...
            if tick: self.lbl_price.setText(f"{tick.bid:.2f}")
        except Exception: pass
