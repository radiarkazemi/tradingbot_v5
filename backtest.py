...
p.add_argument("--commission",type=float, default=0.05,  help="$ per side per 0.01 lot")
args = p.parse_args()

print(f"\n{'='*60}")
print(f"  TraderBot v5 — Backtest  |  {args.symbol}  |  {args.days} days")
print(f"{'='*60}")
print(f"  Balance: ${args.balance:.2f}  Lot: {args.lot}  Count: {args.count}/side")
print(f"  Step: ATR×{args.step}  SL: ATR×{args.sl}  Trail: ATR×{args.trail}")
print(f"  Target: {args.target}%  Daily SL: {args.dailysl}%  Pullback: {args.pullback}%")
print(f"{'='*60}\n")
...
if not mt5.initialize(login=cfg.MT5_LOGIN,
    ...
    print(f"❌ MT5 failed: {mt5.last_error()}"); sys.exit(1)
print("✅ MT5 connected")

mt5.symbol_select(args.symbol, True)
info = mt5.symbol_info(args.symbol)
if info is None:
    print(f"❌ No info for {args.symbol}"); mt5.shutdown(); sys.exit(1)
...
print(f"💵 Pip value: ${pip_val_per_lot:.4f}/pip/lot  (${pip_val_per_lot*args.lot:.6f}/pip/{args.lot}lot)")

# ── Fetch bars ────────────────────────────────────────────────────
now        = datetime.now(tz=timezone.utc)
start_from = now - timedelta(days=args.days + 2)
bars       = mt5.copy_rates_range(args.symbol, mt5.TIMEFRAME_M1, start_from, now)
mt5.shutdown()

if bars is None or len(bars) == 0:
    print("❌ No bars. Make sure MT5 is open with history for this symbol."); sys.exit(1)

cutoff = now - timedelta(days=args.days)
bars   = [b for b in bars if datetime.fromtimestamp(b['time'], tz=timezone.utc) >= cutoff]
print(f"📊 {len(bars):,} M1 bars  |  {cutoff.date()} → {now.date()}\n")

if len(bars) < 50:
    print("❌ Not enough bars."); sys.exit(1)
...
@dataclass
class Pos:
    id: int; side: str; entry: float; sl: float
    trail_d: float; lot: float
    be_done: bool=False; peak: float=0.0

@dataclass
class SR:
    num:int; start_bal:float; end_bal:float; pnl:float
    peak_pnl:float; reason:str; n_pos:int; locked:str
    atr_pip:float; date:str
...
asian_h = 0.0; asian_l = float('inf'); asian_ready = False
daily_start = balance; daily_hit = False; sess_fired = False
last_day    = None

active=False; locked=False; locked_side=""
bal_start=balance; peak=0.0; anchor=0.0
atr_v=0.0; step_d=0.0; sl_d=0.0; trail_d=0.0; be_d=0.0
positions=[]; p_buy=[]; p_sell=[]
sess_num=0; n_win=0; be_set=set(); sess_npos=0; sess_peak=0.0
buy_sl=0.0; sell_sl=0.0
...
def start_session(i, mid, use_asian):
    ...
    active=True; locked=False; locked_side=""
    bal_start=balance; peak=0.0; sess_peak=0.0
    be_set=set(); sess_npos=0; n_win=0
    sess_num += 1
...
def close_session(reason, bid, ask):
    ...
    positions.clear(); p_buy.clear(); p_sell.clear()
    active=False; locked=False; locked_side=""

for i, bar in enumerate(bars):
    bt   = datetime.fromtimestamp(bar['time'], tz=timezone.utc)
    h    = bt.hour; day = bt.date()
    ...
    # Running equity
    fl = sum(pnl(p.side,p.entry,bid if p.side=="BUY" else ask,p.lot) for p in positions)
    equity.append(balance + max(0, fl))

    # New day
    if day != last_day:
        if active: close_session("day end", bid, ask)
        asian_h=0.0; asian_l=float('inf'); asian_ready=False
        daily_start=balance; daily_hit=False; sess_fired=False
        last_day=day

    if daily_hit: continue

    # Daily SL check
    dl = (balance - daily_start)/daily_start*100 if daily_start>0 else 0
    if dl <= -args.dailysl:
        daily_hit=True
        if active: close_session("daily SL", bid, ask)
        continue

    # Asian range
    if h < args.asian:
        if mid > asian_h: asian_h = mid
        if mid < asian_l: asian_l = mid
        asian_ready = asian_h>0 and asian_l<float('inf')
        continue

    # Trade end
    if h >= args.end:
        if active: close_session("trade end", bid, ask)
        continue
    ...
    if not locked or locked_side=="BUY":
        for price in [p for p in p_buy if hi >= p]:
            p_buy.remove(price)
            pos = Pos(pid,"BUY",price+spread_price/2,buy_sl,trail_d,args.lot,peak=price)
            positions.append(pos); pid+=1; sess_npos+=1

    if not locked or locked_side=="SELL":
        for price in [p for p in p_sell if lo <= p]:
            p_sell.remove(price)
            pos = Pos(pid,"SELL",price-spread_price/2,sell_sl,trail_d,args.lot,peak=price)
            positions.append(pos); pid+=1; sess_npos+=1
    ...
    if not locked and positions:
        ...
        if buys and not sells:
            locked_side="BUY"; p_sell.clear(); locked=True
        elif sells and not buys:
            locked_side="SELL"; p_buy.clear(); locked=True
        elif buys and sells:
            if len(buys)>=len(sells):
                locked_side="BUY"
                for p in sells: balance+=pnl("SELL",p.entry,ask,p.lot)
                positions[:]=buys; p_sell.clear()
            else:
                locked_side="SELL"
                for p in buys: balance+=pnl("BUY",p.entry,bid,p.lot)
                positions[:]=sells; p_buy.clear()
            locked=True
    ...
    if positions:
        ...
        for pos in positions:
            ib = pos.side=="BUY"; cur=bid if ib else ask
            if ib:
                if cur>pos.peak: pos.peak=cur
            ...
            if sb and pos.id not in be_set:
                bsl = snap(pos.entry+tick_size) if ib else snap(pos.entry-tick_size)
                if ib and bsl>pos.sl: pos.sl=bsl; be_set.add(pos.id)
                elif not ib and (pos.sl==0 or bsl<pos.sl): pos.sl=bsl; be_set.add(pos.id)

    # Exits
    if positions:
        fl = sum(pnl(p.side,p.entry,bid if p.side=="BUY" else ask,p.lot) for p in positions)
        if fl>sess_peak: sess_peak=fl
...
# Drawdown
peak_e=args.balance; max_dd=0.0
for eq in equity:
...
print(f"  Pip value     : ${pip_val_per_lot*args.lot:.6f}/pip/{args.lot}lot")
print(f"{'='*60}")
print()
...
BG="#0D1117"; CARD="#161B22"; BORDER="#2A3550"
GR="#00D97E"; RD="#FF4560"; GL="#F5A623"
CY="#00BCD4"; T2="#8B9BB4"; WH="#E8EDF5"

plt.style.use("dark_background")
fig = plt.figure(figsize=(16, 10), facecolor=BG)
fig.suptitle(
    f"TraderBot v5  ·  {args.symbol}  ·  Last {args.days} Days  ·  "
    f"${args.balance:.0f} → ${balance:.2f}  ({total_pct:+.1f}%)",
...
# ── Stat cards ────────────────────────────────────────────────────
cards = [
    ("Final Balance",  f"${balance:.2f}",               GR if total_pnl>=0 else RD),
    ("Total P&L",      f"${total_pnl:+.2f} ({total_pct:+.1f}%)", GR if total_pnl>=0 else RD),
    ("Win Rate",       f"{wr:.0f}%  {nw}W / {nl}L",    CY),
]
for col, (title, val, color) in enumerate(cards):
    ax = fig.add_subplot(gs[0, col])
    ax.set_facecolor(CARD); ax.set_xticks([]); ax.set_yticks([])
    for sp in ax.spines.values(): sp.set_edgecolor(BORDER)
    ax.text(0.5, 0.75, title.upper(), transform=ax.transAxes,
            ha="center", va="center", color=T2, fontsize=9, fontweight="bold")
    ax.text(0.5, 0.30, val, transform=ax.transAxes,
...
for sp in ax_eq.spines.values(): sp.set_edgecolor(BORDER)
ax_eq.set_title("Equity Curve  (session boundaries = vertical lines)", color=T2, fontsize=10, pad=6)

x  = np.arange(len(equity))
eq = np.array(equity)
peak_arr = np.maximum.accumulate(eq)

ax_eq.fill_between(x, args.balance, eq, where=eq>=args.balance, alpha=0.15, color=GR)
ax_eq.fill_between(x, args.balance, eq, where=eq< args.balance, alpha=0.15, color=RD)
...
for sp in ax_dd.spines.values(): sp.set_edgecolor(BORDER)
